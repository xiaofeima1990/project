...
if __name__ == '__main__':
    ...
    prov = input("input your province flag ")
    
    link_path= "C:/Users/gum27/Documents/Dropbox/academic/11_work_dataset/justice auction/link/working/"
...
#    driver=webdriver.Firefox(firefox_profile=profile)
    ...
    try:
        ...
        #               driver.execute_script('window.localStorage.clear();')
                if flag_get_info < 0:
                    raise Exception('An error occurred: can not process '+str(index))
            else:
                continue
    ...
    # http://www.datacarpentry.org/python-ecology-lesson/08-working-with-sql/
            ...
            if (index+1)% 25 ==0:
                ...
                df_INFO1.to_sql(city+"_"+prov+"_"+auction_time_flag, con, if_exists="append")
                df_INFO1=pd.DataFrame(columns=col_name)
                df_INFO2.to_sql(city+"_"+prov+"_"+auction_time_flag, con2, if_exists="append")
                df_INFO2=pd.DataFrame(columns=col_bid2)
                driver.quit()
                time.sleep(4)
                profile=webdriver.firefox.firefox_profile.FirefoxProfile()
