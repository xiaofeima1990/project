...
class Update_rule:
    ...
    def l_bound(self,state):
        ...
        drop_info_v[k,:]=self.HS_system(info_struct,self.MU,self.SIGMA2)
    ...
    def HS_system(self,info_struct,MU,Sigma):
        ...
        return np.append(x_drop,info_struct[1:,:])
    ...
    def real_bid(self,xi,bid,state,price_v):
        ...
        try:
            E_j=sum(AA_j*self.truc_x(self.MU,np.diag(self.SIGMA2),x_j_lower,upper_b_j[:,0]))
    ...
    def u_bound_E(self,bid,state):
        ...
        pos     = bid*np.ones(sefl.N-1)
        pos     = pos.reshape(pos.size,1)
        price_v = bid_price*np.ones(sefl.N-1)
        price_v = price_v.reshape(price_v.size,1)

        
        info_struct=np.concatenate((bid,bid_price,self.xi_rival_mu,self.xi_rival_sigma2,self.vi_rival_mu,self.vi_rival_sigma2),axis=1)
        # temp[::-1].sort() sorts the array in place
        info_struct=info_struct[info_struct[:,0].argsort()[::-1]]
        ...
        try:
            ...
            drop_info_jj[jj,:]=self.HS_system(info_struct,self.MU,self.SIGMA2)
