# -*- coding: utf-8 -*-
"""
Created on Wed Jan 09 22:18:03 2019
...
#    print('# of auctions: '+str(TT) )
    ...
    with open('para_est-bashin.txt', 'a+') as f:
        for item in Theta0:
            f.write("%f\t" % item)
...
if __name__ == '__main__':
    ...
    bnds = ((-1, 1), (0,1), (0,1), (0,1))
    xi_n =rng_generate(np.random.RandomState(rng_seed),JJ,max_N)
    
    print("------------------------------------------------------------------")
    print("optimization Begins at : "+ str(now.strftime("%Y-%m-%d %H:%M")))
    print("------------------------------------------------------------------")
    minimizer_kwargs = {"method": "Nelder-Mead","args": (Est_data,d_struct,xi_n)}
    res = basinhopping(GMM_Ineq_parall, Theta, minimizer_kwargs=minimizer_kwargs,niter=1200) 
