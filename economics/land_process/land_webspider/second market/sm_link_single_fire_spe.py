# -*- coding: utf-8 -*-
"""
Created on Mon Jan 18 20:18:20 2016
...
## 二级市场数据抓取，这个程序是抓取所有信息，没有进行土地类别的选择。因此间隔时间较短。
...
def execute_link(driver, i, check_flag,url,temp_date1,temp_date2):
    ...
    while flag != 1:
        try:
#            driver.find_element_by_xpath("//a[@onclick=\"QueryAction.GoPage('TAB',"+str(i)+")\"]").click()
...
#            driver.execute_script("QueryAction.GoPage('TAB',"+str(i)+",200)")            
            ...
            if i >10:             
                data_check=driver.find_element_by_xpath("//td[@id='mainModuleContainer_1438_1439_1801_tdExtendProContainer']/table/tbody/tr/td/table/tbody/tr[2]/td/div/table/tbody/tr/td[2]/span[2]").text
            else:
                data_check=driver.find_element_by_xpath("//td[@id='mainModuleContainer_1438_1439_1801_tdExtendProContainer']/table/tbody/tr/td/table/tbody/tr[2]/td/div/table/tbody/tr/td[2]/span").text
        except  S_exceptions.InvalidSelectorException as e:
            print(e, "try again")
        except Exception as e :
            print(e,"try again strat from beginning-executelink")
...
#            e_email.send_mail("error in catching url!","an error happens")
    ...
    #------------------------------------------------------------------------------------------------------------------------------------------#    
        ...
        temp_date2 = temp_date1 + timedelta(days=366)
        ## consistent within month 
        t_y  = temp_date1.year
        t_y2 = temp_date2.year
        if t_y2!=t_y :
...
if len(input_info)==2:
    ...
    temp_date2=temp_date1+ timedelta(days=731)
