# -*- coding: utf-8 -*-
"""
Created on Mon Jan 18 16:32:27 2016
...
def open_page(driver,url,temp_date1,temp_date2):
    ...
    driver.find_element_by_id("TAB_QueryConditionItem276").click()
    driver.find_element_by_id("TAB_queryTblEnumItem_276").click()
    ...
    driver.find_element_by_id("TAB_queryDateItem_277_1").send_keys(temp_date1.strftime("%Y-X%m-X%d").replace('X0','X').replace('X',''))
...
def execute_link(driver, i, check_flag,url,temp_date1,temp_date2):
    flag=3
    count=5
    if not driver.find_element_by_css_selector('td.pager > input[type=\"text\"]'):
...
def catch_url(driver, url, temp_date1, temp_date2,date_end,error_flag,lock, start_point=1):
    '''
    ...
    while (temp_date2 <= date_end):
        ...
        #------------------------------------------------------------------------------------------------------------------------------------------#
        print("current date range is :",str(temp_date1.strftime("%Y-%m-X%d"))+"---"+str(temp_date2.strftime("%Y-%m-X%d")))
...
def parallel_work(ID,url,temp_date1,temp_date2,date_end,error_sub_info,lock):
    print("ID: %d" % ID)
    driver = webdriver.Firefox()
    
    run_flag=0
    while run_flag==0:
        result_info=catch_url(driver, url, temp_date1, temp_date2, date_end,error_sub_info,lock)
...
#    driver = webdriver.Ie("IEDriverserver.exe")
    ...
    if len(input_info)!= 2 :
        ...
        split_interval=5
        split_num=int(total_days/split_interval)+1
        
        
        for i in range(split_num):
            temp_date1=date_start+timedelta(days=i*split_interval)
            if temp_date1>date_end:
                break
            temp_date2=temp_date1+ timedelta(days=split_interval)
            temp_date3=date_start+timedelta(days=(i+1)*split_interval)
            if temp_date3 > date_end:
                temp_date3=date_end        
            if temp_date2 > temp_date3:
                temp_date2=temp_date3
            
            pool.apply_async(parallel_work, (i, url, temp_date1, temp_date2, temp_date3,error_sub_info,lock))   #维持执行的进程总数为processes=4，当一个进程执行完毕后会添加新的进程进去
            temp_date1=temp_date1+timedelta(days=1)
        ...
        (nrow,ncol)=error_info.shape
        for i in range(0,nrow,1):
            (ID,date_start,date_end,num_page,num_link,Time)=error_info.loc[i,:]
            date_start = datetime.strptime( date_start, '%Y-%m-%d')
            date_end   = datetime.strptime( date_end, '%Y-%m-%d')
            error_sub_info=('y',num_page,num_link,ID)
            pool.apply_async(parallel_work, (i, url, date_start, date_end, date_end,error_sub_info,lock))
